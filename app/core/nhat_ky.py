...
def doc_nhat_ky(limit: int = 50, chi_tu_choi: bool = False) -> list[dict]:
    ...
    `chi_tu_choi=True` la thu NextFarm can khi demo: loc ra dung nhung ca
    he thong da CHAN, de thay co che hoat dong that chu khong phai loi hua.
    """
...
def tong_quan() -> dict[str, Any]:
    ...
    Chi phi tinh tu bang gia CO NGAY TRA CUU (app/services/llm/gia.py).
    Model khong co trong bang thi tra ve None chu KHONG uoc luong bua - mot
    con so chi phi sai lam hong ca cuoc trao doi voi khach hang (muc 37.5).
    """
    ...
    with ket_noi() as con, con.cursor() as cur:
        ...
        # Latency tong moi luot = tong cac chang trong JSONB.
        cur.execute(
        ...
        # Trung binh TUNG CHANG - day moi la thu muc 21.3 can.
        cur.execute(
    ...
    return {
        ...
        # Ti va To cua cong thuc chi phi muc 37.5
...
def thong_ke_kho() -> dict[str, Any]:
    """So lieu kho tri thuc - de NextFarm thay CO CHE DUYET la co that.

    Con so quan trong nhat o day la chenh lech giua `chunk` va
    `chunk_index_duoc`: no cho thay DEC-005 dang chan that, khong phai loi
    hua trong tai lieu.
    """
    with ket_noi() as con, con.cursor() as cur:
