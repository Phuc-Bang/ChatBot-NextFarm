...
> **Trạng thái kiểm thử:** 391 Unit Tests thu thập — 355 chạy được, 36 bỏ qua vì cần PostgreSQL · Working Tree Clean
...
* **Bộ kiểm thử tự động:** **391 Unit Tests thu thập, 355 chạy được, 0 thất bại** — độ phủ từ chuẩn hóa, định tuyến, phân mảnh, truy xuất lai, kiểm duyệt fact, đến mô phỏng pipeline qua HTTP thật. 36 ca còn lại đụng PostgreSQL trực tiếp và **tự bỏ qua** khi không kết nối được, nên con số đúng là *355 chạy được / 36 bỏ qua*, không phải *391/391 PASSED*.
...
1. **Giai đoạn 1.5 (Production Packaging):** Đóng gói Docker Compose / Helm Chart cho Kubernetes, kết nối CI/CD tự động chạy 391 tests khi triển khai.
