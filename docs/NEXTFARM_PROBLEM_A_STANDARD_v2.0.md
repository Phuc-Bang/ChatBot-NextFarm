...
| 9 | Ngưỡng tin cậy Intent Router | **CÒN MỞ — đã hết bị chặn, chưa quét** | Lớp LLM few-shot §11.3 **đã xây xong** (`LLMFewShotRouter`, `router.py:577`, 6 test) nhưng **chưa nối vào `pipeline.py`**, nên đường sống vẫn là rule thuần và nhánh `agronomy_knowledge` vẫn mang `do_tin_cay = 0` — *"lớp rule không biết"*, không phải *"độ tin cậy bằng 0"*. Trục để quét giờ đã có; quét được ngay sau khi nối. Lưu ý: nối vào là mọi số C0/C1/C2 phải đo lại, vì chúng đo trên router thuần rule |
